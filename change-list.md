...
Fix post-revisión: Isabel detectó en Chrome que el texto "¡Nuevo!" no se leía sobre el fueguito. Causa: el bbox natural de ese path (14×21.8 dentro de un viewBox 24×24) es más estrecho que el badge cuadrado de 76×76px, así que el fueguito no llegaba a los bordes laterales — el texto, que sí ocupa casi todo el ancho, caía parcialmente sobre fondo blanco/transparente en vez de sobre naranja, y se volvía invisible ahí. La estrella original no tenía este problema porque su bbox llegaba casi a los bordes del viewBox. Fix: viewBox recortado al bbox real del path + preserveAspectRatio="none" para que el fueguito se estire y rellene el badge de borde a borde, igual que hacía la estrella. Verificado en el sitio real (no en un test aislado) que el texto ya se lee limpio.
