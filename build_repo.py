...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .ticker {
    ...
    overflow: hidden; white-space: nowrap; position: relative; cursor: pointer;
  }
...
function heroEmptyStateHtml() {
  return `<div class="fp-empty-hero">
    ...
    <p>Aún no tienes ningún artículo con 5 estrellas.<br>Valora tus favoritos para que aparezcan aquí arriba.</p>
  </div>`;
}
...
function secondaryEmptyStateHtml() {
  return `<div class="fp-empty-hero">
    <div class="fp-empty-icon">${ICONS.star}</div>
    <p>Todavía no hay artículos con 4 o 3 estrellas.</p>
  </div>`;
}
...
function renderPortada() {
  ...
  const hero = list.filter(a => rateOf(a) === 5).sort((x, y) => (y.date_added || '').localeCompare(x.date_added || ''));
  const secondary = list.filter(a => rateOf(a) === 4 || rateOf(a) === 3).sort((x, y) => {
    const rx = rateOf(x), ry = rateOf(y);
    if (ry !== rx) return ry - rx;
    return (y.date_added || '').localeCompare(x.date_added || '');
  });
  const leftover = list.filter(a => rateOf(a) <= 2);
...
function renderSection(slug) {
  ...
  const subnavHtml = subsections.length
    ? `<div class="subnav">${subsections.map((s,i) => `<a href="#subsec-${i}" class="subnav-pill">${s}</a>`).join('')}${hasOtros ? `<a href="#subsec-otros" class="subnav-pill">Otros</a>` : ''}</div>`
    : '';
  ...
  groups.forEach((sub, i) => {
    const items = list.filter(a => (a.subsection || null) === sub).sort((x,y)=>(y.date_added||'').localeCompare(x.date_added||''));
    if (items.length === 0) return;
    if (sub) html += `<div class="subsection-title" id="subsec-${i}">${sub}</div>`;
    html += railHtml(items);
  });
  if (subsections.length) {
    const rest = list.filter(a => !a.subsection).sort((x,y)=>(y.date_added||'').localeCompare(x.date_added||''));
    if (rest.length) html += `<div class="subsection-title" id="subsec-otros">Otros</div>${railHtml(rest)}`;
  }
