...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .front-lead .fp-hero p {
    font-size: 14.5px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;
  }
  ...
  .front-lead .fp-hero.no-media .fp-body { padding: 20px 22px; gap: 8px; }
  ...
  /* Cada término es una mini-card plegable: clic para expandir la definición ahí mismo,
     sin salir de la home. */
  .ency-mini-row { border-bottom: 1px solid #e5e5e5; }
  .ency-mini-row:last-child { border-bottom: none; }
  .ency-mini-row.open { background: rgba(255,90,31,0.05); }
  .ency-mini-toggle {
    width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 8px;
    padding: 12px 14px; border: none; background: transparent; cursor: pointer; text-align: left;
    font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 13.5px; color: var(--ink);
  }
  .ency-mini-row:hover .ency-mini-toggle { color: var(--accent); }
  .ency-mini-caret { flex: none; width: 11px; height: 11px; color: var(--muted); transition: transform .15s ease; }
  .ency-mini-caret svg { width: 100%; height: 100%; transform: rotate(90deg); }
  .ency-mini-row.open .ency-mini-caret { color: var(--accent); }
  .ency-mini-row.open .ency-mini-caret svg { transform: rotate(-90deg); }
  .ency-mini-def { padding: 0 14px 14px; }
  .ency-mini-def p { margin: 0 0 8px; font-size: 12.5px; line-height: 1.5; color: var(--ink); }
  .ency-mini-def .src {
    font-family: 'Space Mono', monospace; font-size: 10px; text-transform: uppercase; letter-spacing: .5px;
    color: var(--accent); cursor: pointer; text-decoration: none;
  }
  .ency-mini-def .src:hover { text-decoration: underline; }
  ...
  @media (max-width: 900px) {
    ...
    .front-lead .fp-hero, .front-lead .fp-hero.no-media, .ency-sidebar { height: auto; }
    .ency-mini-list { overflow-y: visible; }
  }
...
function paintEncySidebar(items) {
  ...
  wrap.innerHTML = items.map(t => {
    const open = expandedEncyKey === t.key;
    return `<div class="ency-mini-row ${open ? 'open' : ''}" data-key="${t.key}">
      <button class="ency-mini-toggle">
        <b>${t.term}</b>
        <span class="ency-mini-caret">${ICONS.arrow}</span>
      </button>
      ${open ? `
        <div class="ency-mini-def">
          <p>${t.definition}</p>
  ...
  wrap.querySelectorAll('.ency-mini-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      const key = btn.closest('.ency-mini-row').dataset.key;
      expandedEncyKey = expandedEncyKey === key ? null : key;
      paintEncySidebar(items);
    });
  });
  wrap.querySelectorAll('.ency-mini-def [data-open]').forEach(a => {
    a.addEventListener('click', e => { e.stopPropagation(); location.hash = '#/articulo/' + a.dataset.open; });
  });
}
